...
import { Brep2Algo } from "../../../../geometry/algorithm/brep/Brep2Algo";
    ...
    async exec(): Promise<void> {
        ...
        if (this.src.length > 0 && this.des.length > 0) {
            ...
            for (let i = 0; i < edges.length; i++) {
                ...
                let be = Brep2Algo.GetEdgeBeginEndPoint(edges[i].e);
                edges[i].u.assistPoints = [];
                edges[i].u.assistPoints.push({ p: be[0], c: THREE.Color.NAMES.blue });
                edges[i].u.assistPoints.push({ p: be[1], c: THREE.Color.NAMES.blue });
