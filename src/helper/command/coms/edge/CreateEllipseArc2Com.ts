 ...
 * 格式：命令类型 center.x center.y major.x major.y minor.x minor.y u0p.x u0p.y u1p.x u1p.y isForward uuid
 */
class CreateEllipseArc2Com extends ComCreate {
  ...
  async exec(): Promise<void> {
    ...
    // 创建一个曲线段
    let edge = Brep2Builder.BuildEllipseArcEdge2FromCenterBeginEndPoint(this.center, this.major, this.minor);
