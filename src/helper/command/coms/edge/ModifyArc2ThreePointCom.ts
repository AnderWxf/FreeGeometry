...
class ModifyArc2ThreePointCom extends ComModify {
  ...
  async exec(): Promise<void> {
    ...
    if (paras.length >= 3) beginPoint = new Vector2(new Number(paras[1]).valueOf(), new Number(paras[2]).valueOf());
    if (paras.length >= 5) middlePoint = new Vector2(new Number(paras[3]).valueOf(), new Number(paras[4]).valueOf());
    if (paras.length >= 7) endPoint = new Vector2(new Number(paras[5]).valueOf(), new Number(paras[6]).valueOf());
    ...
    if (!beginPoint) beginPoint = userData.assistPoints[0].p as Vector2;
    if (!middlePoint) middlePoint = userData.assistPoints[1].p as Vector2;
    if (!endPoint) endPoint = userData.assistPoints[2].p as Vector2;
