import { Vector2 } from "../../../../math/Math";
import { Transform3 } from "../Transform3";
import { Curve3Data } from "../Curve3Data";
import { GeomType } from "../../../../core/Constents";
/**
 * 3D Hyperbola data struct. 
 *
 */
class Hyperbola3Data extends Curve3Data {
  /**
   * The type of data for unserialize.
   *
   * @type {number}
   */
  private readonly type = GeomType.DATA_TYPE_CURVE3_HYPERBOLA3;
  private readonly typename = GeomType[GeomType.DATA_TYPE_CURVE3_HYPERBOLA3];
  /**
   * The radius value of this Hyperbola3Data.
   *
   * @type {Vector2}
   */
  public radius: Vector2;

  /**
   * Constructs a new 3D Transfrom.
   *
   * @param {Transform3} [trans={position=(0,0,0),rotation=(0,0,0)}}]- The transfrom value of this Hyperbola.
   * @param {Vector2} [radius=(1,1)] - The radius value of this Hyperbola.
   */
  constructor(trans = new Transform3(), radius = new Vector2(1, 1)) {
    super(trans);
    this.radius = radius;
  }

  /**
   * Returns a new Hyperbola3Data with clone values from this instance.
   *
   * @return {Hyperbola3Data} A clone of this instance.
   */
  override clone() {
    let result = super.clone() as Hyperbola3Data;
    result.trans = this.trans.clone();
    result.radius = this.radius.clone();
    return result;
  }

  /**
   * Returns a new Hyperbola3Data with copied values from this instance.
   *
   * @return {Hyperbola3Data} A copy of this instance.
   */
  override copy() {
    return new Hyperbola3Data(this.trans.clone(), this.radius.clone());
  }    

  /**
   * Returns a new Hyperbola3Data with unserialize data.
   *
   * @return {Hyperbola3Data} a new instance.
   */
  static Unserialize(data: any): Hyperbola3Data {
    let ret = new Hyperbola3Data(Transform3.Unserialize(data.trans), Vector2.Unserialize(data.radius));
    ret.uuid = data.uuid;
    return ret;
  }
}

export { Hyperbola3Data };