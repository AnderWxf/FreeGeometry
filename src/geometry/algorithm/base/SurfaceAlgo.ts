...
class SurfaceAlgo {
  ...
  p(uv: Vector2): Vector3 {
    let c = this.ucurve(uv.u);
    let point = new Curve3Algo(c).p(uv.v);
  ...
  /**
   * the D function return r-order partial derivative vector at uv parameter.
  ...
  d(uv: Vector2, r: number = 0): Vector3[] {
    ...
    let du = new Curve3Algo(uc).d(uv.u, r);
    let dv = new Curve3Algo(vc).d(uv.v, r);
    return [du, dv];
  }
  ...
  n(uv: Vector2): Vector3 {
    let uc = this.ucurve(uv.v);
    let vc = this.vcurve(uv.u);
    let tu = new Curve3Algo(uc).t(uv.u);
    let tv = new Curve3Algo(vc).t(uv.v);
    let n = new Vector3();
  ...
  /**
   * the t function return 1-order direction partial derivative vector at uv parameter.
   *
   * @param {Vector2} [uv ∈ R] - the uv parameter of surface.
  ...
  tbn(uv: Vector2): Matrix3 {
    ...
    n.crossVectors(ut, vt).normalize();
    let b = new Vector3();
    b.crossVectors(n, ut).normalize();
    let m = new Matrix3();
    m.extractBasis(ut, b, n);
    return m;
  }
}
