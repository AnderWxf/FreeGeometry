...
class Brep2Builder {
  ...
  /**
   * build parabola edge2 from center a point.
   *
   * @param {Vector2} [c] - The center point.
   ...
   */
  static BuildParabolaEdge2FromCenterAPoint(c: Vector2, a: Vector2, u0: number = 1, u1: number = -1): Edge2 {
    let ret = new Edge2();
   ...
   * @param {Vector2} [c] - The center point.
   * @param {Vector2} [a] - The focus point.
   * @param {Vector2} [b] - The hight point.
   */
  static BuildParabolaEdge2FromCenterABPoint(c: Vector2, a: Vector2, b: Vector2): Edge2 {
    let ret = new Edge2();
    let curve = CurveBuilder.BuildParabola2FromCenterABPoint(c, a);
    let m = curve.trans.makeLocalMatrix().invert();
    b.applyMatrix3(m);
    let u0 = b.x;
    let u1 = -u0;
    ret.u = new Vector2(u0, u1);
  ...
  static Length(curve: Curve2Data, u: Vector2, tol = 0.0001, min: number = 0): number {
    ...
    else if (curve instanceof Parabola2Data) {
      let p = curve.f;
      let t1 = u.y / (2 * p);
      let t0 = u.x / (2 * p);
      // p[t*Math.sqrt(1 + t * t) + Math.log(t + Math.sqrt(1 + t * t))]t0  t1
      let ft0 = p * (t0 * Math.sqrt(1 + t0 * t0) + Math.log(t0 + Math.sqrt(1 + t0 * t0)));
      let ft1 = p * (t1 * Math.sqrt(1 + t1 * t1) + Math.log(t1 + Math.sqrt(1 + t1 * t1)));
      return ft1 - ft0;
    }
