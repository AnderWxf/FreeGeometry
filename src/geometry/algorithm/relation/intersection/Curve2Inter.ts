...
class Curve2Inter {
    ...
    let checkAndPush = (inters: Array<InterOfCurve2>) => {
      for (let j = 0; j < inters.length; j++) {
        ...
        if (Math.abs(g0) <= tol1 && Math.abs(g1) <= tol1) {
          ...
          // Binary(c0a, c1a, inter, tol0, tol1);
          Newton(c0, c1, c0a, c1a, inter, tol0, tol1);
          g0 = c0a.g(inter.p);
          g1 = c1a.g(inter.p);
          if (Math.abs(g0) <= tol1 && Math.abs(g1) <= tol1) {
            if (!isExist(inter.p)) {
          ...
          // Binary(c1a, c0a, inter, tol0, tol1);
          Newton(c0, c1, c0a, c1a, inter, tol0, tol1);
          g0 = c0a.g(inter.p);
          g1 = c1a.g(inter.p);
          if (Math.abs(g0) <= tol1 && Math.abs(g1) <= tol1) {
  ...
  public static CurveXCurve(c0: Curve2Data, c1: Curve2Data, segment: number, tol0: number, tol1: number, n: number = -1, u0: number = 0, u1: number = 1): Array<InterOfCurve2> {
    ...
    for (let i = 0; i < ps.length; i++) {
      ...
      if (pre?.g * cur?.g < 0) {
        let r: InterOfCurve2 = {} as InterOfCurve2;
        if (Bin(algor0, algor1, pre, cur, tol0, tol1, r)) {
          ret.push(r);
        }
        continue;
      }
      ...
      if (i == 0) {
        ...
        if (curg < nexg) {
          let r: InterOfCurve2 = {} as InterOfCurve2;
          if (Close(algor0, algor1, cur, (u1 - u0) / segment, tol0, tol1, r)) {
            ret.push(r);
          }
        }
        continue;
