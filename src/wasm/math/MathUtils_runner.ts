...
import { loadModule } from "./MathUtils_core.bindings";

const lib = await loadModule(new URL("./MathUtils_core.wasm", import.meta.url));
...
function pingpong(x: number, length = 1) {
  ...
  return length - lib.abs(euclideanModulo(x, length * 2) - length);
