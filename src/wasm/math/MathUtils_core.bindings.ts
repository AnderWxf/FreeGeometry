...
export async function loadModule(
  ...
  if (source instanceof ArrayBuffer) {
    ...
    if (typeof globalThis.process !== 'undefined' && globalThis.process.versions?.node) {
      // Node.js / Vitest 环境：直接读文件
      const fs = await import('fs');
      const path = await import('path');
      // 假设 WASM 文件与 bindings.ts 同目录
      const wasmPath = path.resolve(__dirname, 'MathUtils_core.wasm');
      const buffer = fs.readFileSync(wasmPath);
      raw = buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength);
    } else {
      let url = String(source);
      url = url.replace('MathUtils_core.wasm', 'dist/MathUtils_core.wasm')
      raw = await (await fetch(url)).arrayBuffer();
    }
  }
  const { instance } = await WebAssembly.instantiate(raw, importObj as any);
